...
function formatPrice(amount, currency) {
  // If no currency is provided, use the current state currency
  const targetCurrency = currency || state.currency || 'GBP';
...
function initPriceToggles() {
  ...
  domElements.globalCurrencySelector.addEventListener('change', function() {
    ...
    updateAllPrices();
    // Recalculate all toppings to update their costs
    calculateAllToppings();
    // Update the summary to reflect new currency
...
// Update all price displays with the current currency
function updateAllPrices() {
  // Update all price displays with data-price attribute
  const priceDisplays = document.querySelectorAll('.price-display');
  priceDisplays.forEach(display => {
    if (display.dataset.price) {
  ...
  // Recalculate all components to update their prices
  calculateDough();
  calculateZaatarMix();
  calculateCheeseMix();
  calculateMeatMix();
  ...
  // Update the summary if it's currently active
  if (state.activeTab === 'summary') {
    updateSummary();
  }
}
